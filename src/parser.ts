...
const getSubRegionType = (
    ...
    if (currentToken.type === TOKEN_TYPE.DECIMAL)
        return { type: TOKEN_TYPE.DECIMAL };
    if (subRegion && subRegion.type === TOKEN_TYPE.MAGNITUDE)
...
const checkIfTokenFitsSubRegion = (
    subRegion: SubRegion | undefined,
    token: Token,
): { action: HANDLE_TOKEN; type?: TOKEN_TYPE } => {
...
const getSubRegions = (region: Region): SubRegion[] => {
    ...
    let currentSubRegion: SubRegion | undefined;
    const tokensCount = region.tokens.length;
    let i = tokensCount - 1;
    while (i >= 0) {
        const token = region.tokens[i];
        const { action, type } = checkIfTokenFitsSubRegion(
            currentSubRegion,
            token,
        );
...
const matchRegions = (tokens: Token[]): Region[] => {
    ...
    while (i < tokensCount) {
        ...
        const tokenFits: HANDLE_TOKEN = checkIfTokenFitsRegion(
            currentRegion,
            token,
        );
        switch (tokenFits) {
 ...
 * @returns {TOKEN_TYPE}
 */
const getTokenType = (chunk: string): TOKEN_TYPE | undefined => {
...
const tryFixText = (text: string, language: Languages): string => {
    ...
    FIXES[language].forEach(({ from, to, fixables }) => {
        ...
        const resultTextParts = [currentPart];
...
export const parser = (
    text: string,
    options: WithRequired<Options, 'language'>,
): Region[] => {
    const { language, tryFix } = options;
    const finalText = tryFix ? tryFixText(text, language) : text;

    const splitted: string[] = finalText.split(/(\s|[[:punct:]]|\(|\))/i);
    const tokens: Token[] = splitted.reduce(
        (result: Token[], currentValue: string) => {
            ...
            switch (language) {
                ...
                case Languages['fr-fr']:
                    splitted = modifyFrench(currentValue);
                    break;
            }
...
const mergeSubRegions = (subRegions: SubRegion[]): SubRegion[] => {
    for (let i = 0; i < subRegions.length; i++) {
        ...
        if (i > 0) {
            ...
            if (
                previous.type === TOKEN_TYPE.MAGNITUDE &&
                compileSubRegion(previous).sum < compileSubRegion(current).sum
            ) {
